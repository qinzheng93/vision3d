...
class VNLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        dot_prod = (x * d).sum(2, keepdim=True)
        mask = (dot_prod >= 0).float()
        d_norm_sq = (d * d).sum(2, keepdim=True)
        x_out = self.negative_slope * x + (1 - self.negative_slope) * (
            mask * x + (1 - mask) * (x - (dot_prod / (d_norm_sq + EPS)) * d)
        )
        return x_out
...
class VNLinearLeakyReLU(nn.Module):
    def __init__(self, in_channels, out_channels, dim=5, share_nonlinearity=False, negative_slope=0.2):
        ...
        self.map_to_feat = nn.Linear(in_channels, out_channels, bias=False)
        self.batch_norm = VNBatchNorm(out_channels, dim=dim)
    ...
    def forward(self, x):
        ...
        # LeakyReLU
        d = self.map_to_dir(x.transpose(1, -1)).transpose(1, -1)
        dot_prod = (p * d).sum(2, keepdims=True)
        mask = (dot_prod >= 0).float()
        d_norm_sq = (d * d).sum(2, keepdims=True)
        x_out = self.negative_slope * p + (1 - self.negative_slope) * (
            mask * p + (1 - mask) * (p - (dot_prod / (d_norm_sq + EPS)) * d)
        )
        return x_out
...
class VNLinearAndLeakyReLU(nn.Module):
    def __init__(
        self, in_channels, out_channels, dim=5, share_nonlinearity=False, use_batchnorm="norm", negative_slope=0.2
    ):
        super().__init__()
        self.dim = dim
...
class VNStdFeature(nn.Module):
    def __init__(self, in_channels, dim=4, normalize_frame=False, share_nonlinearity=False, negative_slope=0.2):
        ...
        self.vn1 = VNLinearLeakyReLU(
            in_channels, in_channels // 2, dim=dim, share_nonlinearity=share_nonlinearity, negative_slope=negative_slope
        )
    ...
    def forward(self, x):
        ...
        if self.dim == 4:
            ...
            raise RuntimeError("Unsupported dim: {}.".format(self.dim))
