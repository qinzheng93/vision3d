...
from vision3d.array_ops import isotropic_registration_error, point_cloud_overlap
from vision3d.array_ops.knn import knn
from vision3d.array_ops.se3 import apply_transform, get_rotation_translation_from_transform
from vision3d.array_ops.so3 import rotation_matrix_to_quaternion
...
def get_gt_logs_and_infos(gt_dir, num_fragments):
    gt_logs = read_log_file(osp.join(gt_dir, "gt.log"))
    gt_infos = read_info_file(osp.join(gt_dir, "gt.info"))
...
def calibrate_ground_truth(src_pcd, tgt_pcd, transform, voxel_size=0.006):
    src_pcd = src_pcd.voxel_down_sample(0.01)
    tgt_pcd = tgt_pcd.voxel_down_sample(0.01)
    src_points = np.asarray(src_pcd.points)
    tgt_points = np.asarray(tgt_pcd.points)

    # compute overlap
    overlap = point_cloud_overlap(src_points, tgt_points, transform, positive_radius=voxel_size * 5)

    # compute info
    aligned_src_points = apply_transform(src_points, transform)
    nn_distances, nn_indices = knn(tgt_points, aligned_src_points, return_distance=True)
    ...
    if src_corr_points.shape[0] > 0:
        g = np.zeros(shape=(src_corr_points.shape[0], 3, 6))
        g[:, :3, :3] = np.eye(3)
