...
void gather_forward_cuda_launcher(const at::Tensor& sources, const at::Tensor& indices, at::Tensor& targets) {
  ...
  int num_samples = indices.size(1);

  dim3 grid_dim(GET_BLOCKS(num_samples, THREADS_PER_BLOCK), num_channels, batch_size);
  dim3 block_dim(THREADS_PER_BLOCK);

  AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::Bool, sources.scalar_type(), "gather_forward_kernel", [&] {
    gather_forward_kernel<scalar_t><<<grid_dim, block_dim, 0, stream>>>(
        batch_size,
        num_channels,
        num_sources,
        num_samples,
...
void gather_backward_cuda_launcher(
  ...
  int num_samples = target_grads.size(2);

  dim3 grid_dim(GET_BLOCKS(num_samples, THREADS_PER_BLOCK), num_channels, batch_size);
  dim3 block_dim(THREADS_PER_BLOCK);

  AT_DISPATCH_FLOATING_TYPES(target_grads.scalar_type(), "gather_backward_kernel", [&] {
    gather_backward_kernel<scalar_t><<<grid_dim, block_dim, 0, stream>>>(
        batch_size,
        num_channels,
        num_sources,
        num_samples,
