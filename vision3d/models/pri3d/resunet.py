...
from .resnet import create_resnet10, create_resnet18, create_resnet50
...
class Decoder(nn.Module):
    def __init__(self, block_channel, output_channel=1):
        ...
        # last part
        self.conv0 = nn.Conv2d(num_features, output_channel, kernel_size=1, stride=1, padding=0, bias=True)
...
class DecoderMultiRes(nn.Module):
    def __init__(self, block_channel, output_channel=1):
        ...
        # last part
        self.conv1 = nn.Conv2d(num_features, output_channel, kernel_size=1, stride=1, padding=0, bias=True)

        self.up4 = _UpProjection(num_input_features=num_features, num_output_features=num_features // 2)
        num_features = num_features // 2

        # last part
        self.conv2 = nn.Conv2d(num_features, output_channel, kernel_size=1, stride=1, padding=0, bias=True)
...
class Res50UNet(nn.Module):
    ...
    def __init__(self, output_channel, pretrained=False):
        super().__init__()
        resnet_backbone = create_resnet50(num_classes=None, pretrained=pretrained)
...
class Res18UNet(nn.Module):
    ...
    def __init__(self, output_channel, pretrained=False):
        ...
        resnet_backbone = create_resnet18(nclasses=None, pretrained=pretrained)
...
class Res10UNet(nn.Module):
    ...
    def __init__(self, output_channel, pretrained=False):
        ...
        block_channel = [64, 128, 256, 512]
        resnet_backbone = create_resnet10(num_classes=None, pretrained=pretrained)
