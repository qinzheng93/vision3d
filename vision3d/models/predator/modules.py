...
def get_graph_feature(idx, feats, k=10):
    """
...
class SelfAttention(nn.Module):
    ...
    def forward(self, coords, features):
        """
        Here we take coordinates and features, feature aggregation are guided by coordinates
        Input:
            coords:     [B, 3, N]
            feats:      [B, C, N]
        Output:
            feats:      [B, C, N]
        """
        idx = knn(coords, coords, self.k, remove_nearest=True, transposed=True)  # (B, N, k)
        ...
        x1 = get_graph_feature(idx, x0.squeeze(-1), k=num_neighbors)
        x1 = F.leaky_relu(self.in1(self.conv1(x1)), negative_slope=0.2)
        x1 = x1.max(dim=-1, keepdim=True)[0]

        x2 = get_graph_feature(idx, x1.squeeze(-1), k=num_neighbors)
        x2 = F.leaky_relu(self.in2(self.conv2(x2)), negative_slope=0.2)
        x2 = x2.max(dim=-1, keepdim=True)[0]

        x3 = torch.cat((x0, x1, x2), dim=1)
        x3 = F.leaky_relu(self.in3(self.conv3(x3)), negative_slope=0.2).view(batch_size, -1, num_points)

        return x3


def MLP(channels: list, normalization=True):
...
class MultiHeadedAttention(nn.Module):
    """Multi-head transformer to increase model expressivitiy"""
...
class GCN(nn.Module):
    """
    Alternate between self-transformer and cross-transformer
    Input:
        coords:     [B, 3, N]
        feats:      [B, C, N]
    Output:
